...
const firebaseConfig = {
  apiKey: 'AIzaSyDMkDI_cXEJQpMJ8khpHiDBmorXQ-eHsmY',
  authDomain: 'checklist-facilities.firebaseapp.com',
  projectId: 'checklist-facilities',
  storageBucket: 'checklist-facilities.firebasestorage.app',
  messagingSenderId: '524756030943',
  appId: '1:524756030943:web:66bee4210714398b42a06a',
  measurementId: 'G-BBQL75MXCH',
};
...
const deleteCollectionDocs = async (db, collectionName) => {
...
const updateAuthUI = () => {
  ...
  if ($resetAll) $resetAll.style.display = locked ? 'none' : 'inline-flex';
...
const resetAllData = async () => {
  if (!isLoggedIn) {
    alert('Faça login para resetar todos os dados.');
    return;
  }
  const confirmed = window.confirm('Apagar todo o checklist e histórico (local e Firestore)? Esta ação não pode ser desfeita.');
  if (!confirmed) return;
  ...
  render();
  renderHistory();
  setSyncStatus('Dados limpos localmente. Sincronizando com a nuvem...', 'syncing');

  try {
    const db = await initFirestore();
    if (!db) {
      setSyncStatus('Reset local concluído. Firestore indisponível.', 'error');
      return;
    }

    await deleteCollectionDocs(db, FIRESTORE_COLLECTION);
    await deleteCollectionDocs(db, FIRESTORE_IMAGES_COLLECTION);
    await db.collection(FIRESTORE_STATE_COLLECTION).doc(FIRESTORE_STATE_DOC).set({ subjects: [] });
    setSyncStatus('Checklist e histórico resetados.', 'online');
  } catch (err) {
    logError('resetAllData', err);
    setSyncStatus('Erro ao resetar dados', 'error');
  }
};
